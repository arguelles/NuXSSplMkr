...
*/
         std::vector<double> e_range {1.00000000e+08};
  ...
  for (Current IT : {CC,NC}) {
    ...
    for (NeutrinoType neutype : {neutrino,antineutrino}){
      ...
      for (PDFVar pdfvar : {central,minus,plus}){
        ...
        std::string filename_dsdy = static_cast<std::string>(SAVE_PATH) + "test_dsdE-"+NeutrinoTypeLabel[neutype]+"-N-"+IntTypeLabel[IT]+"-"+pdfname+"_"+PDFVarLabel[pdfvar]+".dat";
        std::string filename_sigma = static_cast<std::string>(SAVE_PATH) + "test_sigma-"+NeutrinoTypeLabel[neutype]+"-N-"+IntTypeLabel[IT]+"-"+pdfname+"_"+PDFVarLabel[pdfvar]+".dat";
        ...
        for( double enu : e_range){
          ...
          //std::cout << "done with energy " << enu << std::endl;
        }
