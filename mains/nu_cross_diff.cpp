  ...
  for (Current IT : {CC,NC}) {
    ...
    for (NeutrinoType neutype : {neutrino,antineutrino}){
      ...
      for (PDFVar pdfvar : {central,minus,plus}){
        ...
        std::cout << "BEGIN d-sigma-"+NeutrinoTypeLabel[neutype]+"-N-"+IntTypeLabel[IT]+"-"+pdfname+"_"+PDFVarLabel[pdfvar]+".dat" << std::endl;
        ...
        std::cout << "END d-sigma-"+NeutrinoTypeLabel[neutype]+"-N-"+IntTypeLabel[IT]+"-"+pdfname+"_"+PDFVarLabel[pdfvar]+".dat" << std::endl;
      }
    }
  }
